...
export const runBacktestFn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const result: BacktestResult = runBacktest(candles, config);
    const label = data.label?.trim() || `${data.timeframe}m · ${data.period} · ${data.rrTarget}R`;
    ...
    if (data.save) {
      const { data: row } = await context.supabase
        ...
        .insert({
          ...
          bars: result.bars,
          config: { ...config, period: data.period } as any,
